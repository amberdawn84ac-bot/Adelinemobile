...
const CREEK_DISCOVERIES: Discovery[] = [
  {
    id: 'fish', x: 53, y: 74, mark: '⌇', title: 'Something in the shallows',
    body: 'Three small fish are floating belly-up where the creek bends. The water here has a faint cloudy sheen.',
    note: 'Dead fish + cloudy water. Check upstream before deciding what caused it.', accent: '#315f70'
  },
  {
    id: 'bottle', x: 61, y: 62, mark: '◇', title: 'A blue glass bottle',
    body: 'A little cobalt bottle is caught between two roots. The label is gone, but there is a sharp mineral smell around the cap.',
    note: 'Unknown bottle upstream. Do not assume it is the source. Keep looking for a route into the creek.', accent: '#345f91'
  },
  {
    id: 'pipe', x: 67, y: 51, mark: '◒', title: 'The old drain',
    body: 'Behind nettles and stones, an iron pipe disappears beneath the road. Damp orange residue marks the soil below it.',
    note: 'Drainage pipe enters the creek. Orange residue suggests minerals or rust, but the source is still unknown.', accent: '#9a4e35'
  },
  {
    id: 'paper', x: 31, y: 42, mark: '⌁', title: 'A torn delivery slip',
    body: 'A rain-softened scrap has been pinned under a stone. Most of the ink is gone, but one line remains: “Mill Road — drums — Thursday.”',
...
export default function HubWorld({
  ...
  useEffect(() => {
    localStorage.setItem('adeline_creek_clues', JSON.stringify(foundIds))
  }, [foundIds])
  ...
  const nearbyDiscovery = CREEK_DISCOVERIES.find(d =>
    distance(player.x, player.y, d.x, d.y) < DISCOVERY_PROXIMITY
  )
  ...
  return (
    <div className="relative w-full h-full overflow-hidden select-none bg-[#ebe3cf] text-[#2d2924] touch-none">
      ...
      {littleTrees.map((t,i) => (
        <div key={i} className="absolute" style={{ left:`${t.x}%`, top:`${t.y}%`, transform:`scale(${t.s})`, zIndex:5 }}>
          <div className="relative w-12 h-16 -translate-x-1/2">
            ...
            <span className="absolute left-0 top-[23px] w-8 h-8 rounded-full border border-[#2e332b]/50 bg-[#536b51]/45" />
          </div>
        </div>
      ))}
      ...
      {/* The first adventure is hidden in the scenery, not announced by a lesson card. */}
      {CREEK_DISCOVERIES.map(d => {
        ...
        return (
          <button
            key={d.id}
            type="button"
            onClick={() => { setOpenDiscovery(d); setFoundIds(prev => prev.includes(d.id) ? prev : [...prev, d.id]) }}
            className="absolute z-[17] flex items-center justify-center rounded-full transition-all"
            style={{ left:`${d.x}%`, top:`${d.y}%`, width: close ? 32 : 20, height: close ? 32 : 20, transform:'translate(-50%,-50%)', color:d.accent }}
            aria-label={`Inspect ${d.title}`}
          >
            <span className={`${close ? 'opacity-100 scale-110' : found ? 'opacity-45' : 'opacity-20'} text-xl drop-shadow`}>{d.mark}</span>
            {close && <span className="absolute inset-0 rounded-full border border-current animate-ping opacity-25" />}
          </button>
        )
      })}
      ...
      {TOWN_BUILDINGS.map(building => (
        <TownBuilding
          key={building.id}
          building={building}
          isNearby={nearbyBuilding?.id === building.id && !nearbyDiscovery}
      ...
      <div className="absolute flex flex-col items-center" style={{ left:`${player.x}%`, top:`${player.y}%`, transform:'translate(-50%,-50%)', zIndex:30, filter:'drop-shadow(0 6px 8px rgba(35,30,23,.3))' }}>
      ...
      {activeNearby && !pickerBuilding && !openDiscovery && (
        <div className="absolute left-1/2 bottom-[88px] -translate-x-1/2 z-40 pointer-events-none">
          <div className="px-4 py-2 rounded-full bg-[#f5efdf]/92 border border-[#332d25]/20 shadow-lg text-[11px] font-serif whitespace-nowrap">
            {nearbyDiscovery ? 'Something here catches your eye.' : nearbyBuilding && nearbyBuilding.unlockXP > currentXP ? 'The way is closed for now.' : `${nearbyBuilding?.name} is close.`}
          </div>
        </div>
      )}

      {allCluesFound && !openDiscovery && !pickerBuilding && (
        <button
          onClick={() => onEnterBuilding('the_makers_lab', 'build_it', 'CREATION_SCIENCE', 'The creek is cloudy, fish are dying, an old drainage pipe has orange residue, and a delivery slip mentions drums on Mill Road. Design a way to test the water and determine what evidence would distinguish contamination from harmless sediment or rust.')}
          className="absolute left-1/2 top-[18%] -translate-x-1/2 z-40 max-w-[280px] px-5 py-3 rounded-[18px] border border-[#372f27]/20 bg-[#f3ebd8]/95 shadow-xl text-left"
        >
          <span className="block text-[9px] tracking-[.23em] uppercase text-[#6e655a]">field note</span>
          <span className="block font-serif text-[15px] text-[#2e2922] mt-1">You have enough clues to test the creek instead of guessing.</span>
          <span className="block text-[10px] text-[#6a5f52] mt-2">Take the evidence to the workshop →</span>
        </button>
      )}

      {/* Hold controls: movement continues while a thumb stays down. */}
      <div className="absolute left-4 bottom-4 z-40 sm:hidden w-[122px] h-[122px] rounded-full bg-[#efe7d4]/35 border border-[#332d25]/10 backdrop-blur-[1px]">
        {[
          {k:'ArrowUp', label:'↑', cls:'left-[43px] top-[5px]'},
          {k:'ArrowLeft', label:'←', cls:'left-[5px] top-[43px]'},
          {k:'ArrowRight', label:'→', cls:'right-[5px] top-[43px]'},
          {k:'ArrowDown', label:'↓', cls:'left-[43px] bottom-[5px]'},
        ].map(c => (
          <button
            key={c.k}
            aria-label={`Move ${c.label}`}
            onPointerDown={e => { e.currentTarget.setPointerCapture(e.pointerId); startMobileMove(c.k) }}
            onPointerUp={() => stopMobileMove(c.k)}
            onPointerCancel={() => stopMobileMove(c.k)}
            onPointerLeave={() => stopMobileMove(c.k)}
            className={`absolute ${c.cls} w-9 h-9 rounded-full bg-[#f5efdf]/78 border border-[#332d25]/20 shadow-md text-[#3a332b] active:scale-95`}
          >{c.label}</button>
        ))}
      </div>
      ...
      <button
        ...
        disabled={!activeNearby}
        className="absolute right-5 bottom-7 sm:bottom-5 z-40 w-[70px] h-[70px] rounded-full border-2 border-[#3b332a]/25 bg-[#5b3769]/90 text-[#fff8e9] font-serif text-[12px] shadow-[0_8px_24px_rgba(49,35,55,.3)] disabled:opacity-20 disabled:grayscale"
      >
        {nearbyDiscovery ? 'inspect' : 'enter'}
      </button>
      ...
      {openDiscovery && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-[#17140f]/45 backdrop-blur-[1px]" onClick={() => setOpenDiscovery(null)}>
          <div className="w-full sm:max-w-md rounded-t-[30px] sm:rounded-[30px] border border-[#30291f]/20 bg-[#f5eedc] p-6 shadow-[0_24px_80px_rgba(22,18,13,.45)]" onClick={e => e.stopPropagation()}>
            <div className="flex items-start gap-4">
              <div className="w-11 h-11 rounded-full border border-[#332d25]/20 flex items-center justify-center font-serif text-2xl" style={{color:openDiscovery.accent}}>{openDiscovery.mark}</div>
              <div className="flex-1">
                <p className="text-[9px] tracking-[.25em] uppercase text-[#766b5e]">found near the creek</p>
                <h3 className="font-serif text-[24px] leading-tight text-[#2d2924] mt-1">{openDiscovery.title}</h3>
              </div>
              <button onClick={() => setOpenDiscovery(null)} className="text-[#5e554a]/60 text-xl">×</button>
            </div>
            <p className="mt-5 font-serif text-[15px] leading-relaxed text-[#463e34]">{openDiscovery.body}</p>
            <div className="mt-5 border-l-2 pl-4" style={{borderColor:openDiscovery.accent}}>
              <p className="text-[9px] uppercase tracking-[.2em] text-[#766b5e]">your field note</p>
              <p className="mt-1 text-[12px] leading-relaxed text-[#5b5145] italic">{openDiscovery.note}</p>
            </div>
            <div className="mt-6 flex items-center justify-between">
              <span className="text-[10px] text-[#766b5e]">{foundIds.length} of {CREEK_DISCOVERIES.length} clues noticed</span>
              <button onClick={() => setOpenDiscovery(null)} className="px-5 py-2 rounded-full bg-[#5b3769] text-[#fff8e9] font-serif text-[12px]">keep looking</button>
            </div>
          </div>
        </div>
      )}
