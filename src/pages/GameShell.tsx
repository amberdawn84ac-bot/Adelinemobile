...
type GameScreen = 'avatar_builder' | 'hub' | 'mission' | 'brain_battle' | 'kitchen' | 'creek_case'
...
export default function GameShell() {
  ...
  const guestAvatar = guestAvatarRaw && Object.keys(guestAvatarRaw).length > 0 ? parseAvatar(guestAvatarRaw as Record<string, unknown>) : null
  ...
  useEffect(() => { if (activeChild) getSeasonPass(activeChild.id).then(setClaimedTiers) }, [activeChild])
  useEffect(() => { if (activeChild) getLifeMap(activeChild.id).then(setAllEntries) }, [activeChild])
  useEffect(() => {
    if (overlay === 'settings' && activeChild?.town_id && !town && !townLoadFailed) {
      getTown(activeChild.town_id).then(result => result ? setTown(result) : setTownLoadFailed(true))
    }
  }, [overlay, activeChild, town, townLoadFailed])
  useEffect(() => { setTown(null); setTownLoadFailed(false) }, [activeChild?.id])
  useEffect(() => { if (activeChild?.town_id) getTownStorm(activeChild.town_id).then(setStormStatus) }, [activeChild?.town_id])

  async function saveAvatar(avatar: AvatarData) {
    setAvatarData(avatar)
    if (activeChild) await updateStudentProfile(activeChild.id, { avatar_data: avatar as unknown as Record<string, unknown> })
    else if (guestSession) localStorage.setItem('adeline_guest', JSON.stringify({ ...guestSession, avatarData: avatar }))
    setScreen('hub')
  }

  function addXP(amount: number) { setLocalXP(prev => prev + amount); if (activeChild) patchXP(activeChild.id, amount) }
  function addCoins(amount: number) { setLocalCoins(prev => prev + amount); if (activeChild) patchCoins(activeChild.id, amount) }

  async function handleTownFormSubmit() {
    setTownFormError('')
    setTownFormLoading(true)
    try {
      const result = townFormMode === 'create' ? await createTown(townFormValue.trim()) : await joinTown(townFormValue.trim().toUpperCase())
      if (!result) setTownFormError(townFormMode === 'create' ? 'Could not create town. Try again.' : "That code didn't work. Check it and try again.")
      else { setTown(result); setTownFormValue(''); await refreshUser() }
    } finally { setTownFormLoading(false) }
  }
  ...
  function exitToHub() { setCurrentBuilding(null); setScreen('hub') }
  function openJournalPage(page: Exclude<Overlay, null>) { setJournalOpen(false); setOverlay(page) }

  const buildingMeta = currentBuilding ? BUILDING_META[currentBuilding] : null
  const buildingFallbackTracks: Track[] = currentBuilding ? (TOWN_BUILDINGS.find(b => b.id === currentBuilding)?.fallbackMissions[0]?.tracks ?? ['ENGLISH_LITERATURE']) : ['ENGLISH_LITERATURE']
  ...
  if (screen === 'avatar_builder') return <AvatarBuilder initialAvatar={storedAvatar ?? guestAvatar ?? undefined} playerName={playerName} onSave={saveAvatar} />

  if (screen === 'creek_case') {
    return <CreekInvestigation playerName={playerName} onBack={exitToHub} onComplete={handleMissionComplete} />
  }
  ...
  if (screen === 'kitchen') {
    return (
      <div className="relative h-screen w-screen overflow-hidden">
        ...
        <div className="h-full w-full pt-16"><AdelineKitchen studentId={activeChild?.id ?? null} playerName={playerName} gradeBand={gradeBand} onBack={exitToHub} /></div>
      </div>
    )
  }
  ...
  if (screen === 'brain_battle' && currentBuilding) {
    return (
      <div className="relative h-screen w-screen overflow-hidden">
        <BrainBattle studentId={activeChild?.id ?? null} track={resolvedTrack} gradeBand={gradeBand} playerName={playerName} onComplete={(xp, coins) => { addXP(xp); addCoins(coins); exitToHub() }} onBack={exitToHub} />
      </div>
    )
  }
  ...
  if (screen === 'mission' && currentBuilding && buildingMeta) {
    return (
      <div className="relative h-screen w-screen overflow-hidden">
        <RoomMission roomId={currentBuilding} roomLabel={buildingMeta.label} roomEmoji="" roomTracks={[resolvedTrack]} playerName={playerName} systemContext={activityTopic ?? `${activityMode} activity in ${buildingMeta.label}`} studentId={activeChild?.id ?? null} gradeBand={gradeBand} stormMission={activeStormMission} onComplete={handleMissionComplete} onBack={exitToHub} />
      </div>
    )
  }
  ...
  return (
    <div className="relative h-screen w-screen overflow-hidden">
      ...
      <div className="h-full w-full pt-16">
        {stormStatus?.phase === 'warning' && <div className="pointer-events-none fixed left-1/2 top-[66px] z-30 -translate-x-1/2 rotate-[-1deg] rounded-sm border border-[#3a3128]/15 bg-[#efe7d4]/85 px-4 py-2 font-serif text-[10px] italic tracking-wide text-[#51473c] shadow-md backdrop-blur">barometer falling · {stormStatus.days_until_hit} day{stormStatus.days_until_hit === 1 ? '' : 's'}</div>}
        {stormStatus?.phase === 'hit' && <div className="pointer-events-none fixed inset-0 z-20 bg-[#34434a]/12 backdrop-brightness-[.86]" />}
        <HubWorld avatarData={avatarData} playerName={playerName} studentId={activeChild?.id ?? null} currentXP={localXP} onEnterBuilding={enterBuilding} onEnterKitchen={() => setScreen('kitchen')} onStartCreekCase={() => setScreen('creek_case')} onXpEarned={addXP} onCoinsEarned={addCoins} onLifeMapEntry={handleLifeMapEntry} />
      </div>

      <button onClick={() => setJournalOpen(v => !v)} className="fixed right-4 top-[74px] z-40 rotate-[1deg] rounded-[6px_10px_7px_9px] border border-[#382f27]/25 bg-[#eadfc8]/95 px-3 py-2 font-serif text-[11px] text-[#3d352c] shadow-lg">journal</button>
      ...
      {overlay === 'settings' && activeChild && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#241f1a]/45 p-6" onClick={() => { setOverlay(null); setTownLoadFailed(false) }}>
          <div className="w-full max-w-sm space-y-4 rounded-[24px_18px_28px_20px] border border-[#42372e]/20 bg-[#f3ead6] p-6 font-serif text-[#3b332b] shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            {activeChild.parent_id ? <p className="text-sm text-[#675b4e]">Linked to {activeChild.parent_display_name ?? 'a parent'}.</p> : (
            ...
            <div className="space-y-2 border-t border-[#44392f]/15 pt-4">
              ...
              {activeChild.town_id && town ? (
                <div className="space-y-1 text-sm text-[#675b4e]"><p>{town.name}</p><p className="text-xs">{town.members.length} member{town.members.length === 1 ? '' : 's'} · treasury {town.treasury}</p><p className="font-mono text-xs">join code {town.join_code}</p></div>
              ) : activeChild.town_id ? (
                ...
                <div className="space-y-2">
                  ...
                  <input type="text" value={townFormValue} onChange={e => setTownFormValue(townFormMode === 'join' ? e.target.value.toUpperCase().slice(0, 6) : e.target.value)} placeholder={townFormMode === 'create' ? 'town name' : '6-digit code'} className="w-full rounded-lg border border-[#44392f]/20 bg-[#fff9eb]/60 px-3 py-2 text-sm outline-none" maxLength={townFormMode === 'join' ? 6 : 100} />
                  {townFormError && <p className="text-xs text-[#8b4038]">{townFormError}</p>}
                  <button onClick={handleTownFormSubmit} disabled={townFormLoading || !townFormValue.trim()} className="w-full rounded-full bg-[#315d58] py-2 text-sm text-[#fff7e9] disabled:opacity-40">{townFormLoading ? 'one moment…' : townFormMode === 'create' ? 'start town' : 'join town'}</button>
                </div>
              )}
            </div>
      ...
      {overlay === 'portfolio' && <Portfolio entries={allEntries} studentName={playerName} gradeBand={gradeBand} onClose={() => setOverlay(null)} onExport={() => { setOverlay(null); setShowTranscript(true) }} />}
      {showTranscript && <Transcript entries={allEntries} studentName={playerName} gradeBand={gradeBand} parentName={activeChild?.parent_display_name ?? 'Parent'} onClose={() => setShowTranscript(false)} />}
    </div>
  )
}

function JournalLink({ label, onClick }: { label: string; onClick: () => void }) {
  return <button onClick={onClick} className="w-full rounded-lg px-3 py-2 text-left font-serif text-[12px] text-[#453c32] transition hover:bg-[#dfd1b5]/70">{label}</button>
}
